...
RSpec.describe Review do
  ...
  describe 'scope' do
    describe '.search_by' do
      let(:title_to_find) { create(:review, title: 'Title 1') }
      let(:title_not_to_find) { create(:review, title: 'Title 2') }
    ...
    describe '.for_article' do
      ...
      let(:review) { create(:review, article:) }
      let(:different_review) { create(:review) }

      it 'returns comments for the specified article' do
        expect(described_class.for_article(article)).to include(review)
      end

      it 'does not return different comments' do
        expect(described_class.for_article(article)).not_to include(different_review)
      end
    end
  end
end
